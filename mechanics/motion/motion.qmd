...
Skinny Physics is meant to be a lightweight (skinny, after all) presentation of some physics that might help you appreciate some of the posts in **QS&BB.** Maybe you've had a course in your past and maybe not, so this chapter has some bare bones (skinny, after all) **facts about motion**.

So: three sections: (1) Some ideas you might not have had in a previous course. (2) A listing of some motion facts. And, (3) a bit of gentle background behind those facts. If you'd like more, then visit the [full textbook](https://qstbb.pa.msu.edu/storage/ISP220_fall2020/QS&BB2020/intro.html) presentation.

## Now for something different:

::: {.callout-important icon="false"}
🫵 What's different is that even if you know some of the basics of velocity and acceleration, you need to be comfortable with spacetime diagrams in @sec-spacetimediagrams ! That's how we'll pursue Feynman Diagrams.
:::

## Just some motion facts:

**changes in something**

I'll use:

1.  $x_0$ to be where we start in distance
2.  $x$ to be where we end up (sometimes, I'll be explicit and say "$x_f$" for "final.")
3.  We’ll use the Greek symbol Delta, $\Delta$ to mean “change of”…this will come up a lot.
    -   Change always means: the end minus the beginning.
4.  a change in position is $\Delta x = x_{\text{ended up}}-x_{\text{where we started}} = x-x_0$
5.  same goes for time, from $\Delta t= t_{\text{ended up}}-t_{\text{when we started}} = t - t_0$
6.  symbol for average: $<A>$
7.  symbol for the (nearly) constant acceleration due to gravity close to the Earth's surface: $g$, "little g"
...
![](images/clipboard-2908347734.png){#fig-LABEL fig-align="center" width="80%"}

Characterizing motion is simple in principle! You need two things:

![The complete set of tools to characterize motion.](screenshot_3644.png){fig-align="center" width="80%"}
...
\end{align}
$$ {#eq-speeddef} where "$v$" is the velocity and it's defined by the middle equation in equation @eq-speeddef.
...
![CAPTION](screenshot_3766.png){fig-align="center" width="80%"}

Dimensions of acceleration are a little unfamiliar but we can work them out:

![CAPTION](screenshot_3668.png){fig-align="center" width="80%"}
...
![Our platform for describing motion. A ball on a pool table…sitting there minding its own business.](screenshot_3649.png){#fig-balltablestill fig-align="center" width="80%"}

Let's plot it on an x-y axis set.

![A “space-space” diagram for the ball sitting still. Time marches on for it in that position.](screenshot_3656.png){#fig-fig-ballstillss fig-align="center" width="80%"}
...
\end{align}
$$ {#eq-eq1} So if the speed is uniform, like the ball above, then the acceleration is zero (since $\Delta v = 0$).

If the speed changes in time, then we have accelerated motion since $\Delta v \ne 0$.

![CAPTION](screenshot_3669.png){fig-align="center" width="80%"}
...
![CAPTION](screenshot_3670.png){fig-align="center" width="80%"}
